...
.. list-table::
  ...
  * - 4
    ...
    - MUST be reported as an ontology term ID (CURIE), optionally followed by its human-readable label in parentheses in the format ``ID (Label)`` (the label is recommended but not required) -- for example from the `EFO <http://www.ebi.ac.uk/efo/EFO_0000324>`_ ontology -- or the literal value ``Other`` when no ontology term applies
  ...
  * - 5
    ...
    - MUST be reported as an ontology term ID (CURIE), optionally followed by its human-readable label in parentheses in the format ``ID (Label)`` (the label is recommended but not required) -- such as a child of the `GO 'cellular_component' term <http://purl.obolibrary.org/obo/GO_0005575>`_ -- or the literal value ``Other`` when no ontology term applies
